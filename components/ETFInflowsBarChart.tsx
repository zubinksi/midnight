...
export default function ETFInflowsBarChart({ data, height = 160 }: { data: InflowBarPoint[]; height?: number }) {
  ...
  const n      = data.length
  const bars   = [
    { key: 'total' as const, color: ETF_COLORS.total },
    { key: 'bhyp'  as const, color: ETF_COLORS.bhyp  },
    { key: 'thyp'  as const, color: ETF_COLORS.thyp  },
  ]

  if (!w || n === 0) return <div ref={ref} style={{ height }} />

  const groupW  = w / n
  const barW    = Math.max(Math.floor((groupW * 0.7 - 2) / 3), 3)
  const barGap  = 1
  const groupPad = (groupW - (barW * 3 + barGap * 2)) / 2

  function toRect(value: number): { y: number; h: number } {
    const top = PAD_TOP + chartH * (maxVal - Math.max(value, 0)) / range
    const bot = PAD_TOP + chartH * (maxVal - Math.min(value, 0)) / range
